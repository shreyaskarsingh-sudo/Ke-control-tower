...
import { initMcpSession, callMcpTool, extractText } from "@/lib/ke-mcp";
...
export async function POST(request: Request) {
  ...
  const { merchantId, chatName } = await request.json();
  if (!merchantId?.trim()) {
  ...
  try {
    ...
    // Auth using merchant ID only — MCP server handles credentials internally
    ...
    // Fetch client info + campaigns + automations + segments in parallel
...
Be direct and specific. Avoid generic statements.`;
    ...
    return NextResponse.json({ analysis, merchantId, chatName });
  } catch (err) {
